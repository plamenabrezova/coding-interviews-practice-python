...
# at which point we can simply return. However, the challenge is figuring out how to terminate the program if it finds
# a loop. Otherwise, the program would go on forever.
...
# The idea is to have two pointers, the fast pointer (or "hare") moves at double speed of the slow pointer
# (or "tortoise"). Each cycle, the tortoise moves once and the hare moves twice.
...
def has_cycle_my_version(nodes: Node) -> bool:
    seen_nodes = Counter()
    counter = 0

    while nodes.next or counter > 100000:

        if nodes.val not in seen_nodes:
            seen_nodes[nodes.val] = 0
        seen_nodes[nodes.val] += 1

        if nodes.next.val in seen_nodes:
            return True
